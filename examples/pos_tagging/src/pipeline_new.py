...
from transformers.pipelines import SUPPORTED_TASKS, ArgumentHandler
...
class PosTaggingArgumentHandler(ArgumentHandler):
    """
    Handles arguments for POS tagging.
    """

    def __call__(self, inputs: Union[str, List[str]], **kwargs):

        if inputs is not None and isinstance(inputs, (list, tuple)) and len(
                inputs) > 0:
            inputs = list(inputs)
        ...
        if offset_mapping:
            if isinstance(offset_mapping, list) and isinstance(offset_mapping[0],
                                                               tuple):
...
class ExamplePosTaggingPipeline(TokenClassificationPipeline):
    def __init__(
        ...
        args_parser = args_parser or PosTaggingArgumentHandler()
    ...
    def __call__(self, inputs: Union[str, List[str]], **kwargs):
        ...
        # tokens = [item[0] for item in whitespace_tokenizer.pre_tokenize_str(inputs)]
        # processed_input = self.data_processor.text_to_instance(tokens)
        ...
        for i, sentence in enumerate(_inputs):
            ...
            tokens = [item[0] for item in
                      self.whitespace_tokenizer.pre_tokenize_str(sentence)]
            processed_input = self.data_processor.text_to_instance(tokens)
            adapted_instance = self._data_adapter(processed_input)
            ...
            single_instance_batch = self._data_collator.build_model_inputs(
                (indexed_instance,),
                should_eliminate_model_incompatible_keys=False
            )
            self._data_collator.pad(single_instance_batch)
            input_ids = np.asarray(single_instance_batch["input_ids"][0])
            token_type_ids = single_instance_batch["token_type_ids"][0]
            attention_mask = np.asarray(single_instance_batch["attention_mask"])
