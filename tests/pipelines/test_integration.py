...
def integration_expected_multi_inference():
    return [
        ...
        {
            "answer": SpanTuple(
                text="end of the main drive (and in a direct line that connects",
                start=558,
            ),
...
def test_integration(
    ...
    config_path = (
        FIXTURES_ROOT / "pipelines/pipeline_integration_experiment.jsonnet"
    )
    ...
    EXPERIMENT_CONFIG = os.path.join(
        PRETRAINED_MODEL_PATH, "experiment_config.json"
    )
