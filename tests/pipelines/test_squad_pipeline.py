...
def roberta_squad_pipeline_expected_output():
    return {'score': 0.0002498578105587512,
            'answer': SpanTuple(text='Broncos defeated the National Football Conference (', start=184)
            }

def test_roberta_squad_pipeline_execution(roberta_squad_pipeline, roberta_squad_pipeline_sample_input, roberta_squad_pipeline_expected_output):
    actual_output = roberta_squad_pipeline(roberta_squad_pipeline_sample_input)
    diff = DeepDiff(actual_output, roberta_squad_pipeline_expected_output, significant_digits=3)
