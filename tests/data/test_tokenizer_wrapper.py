...
def gpt2_tokenizer_with_context_and_answer_tokens():
    ...
    return TokenizerWrapperWithContextAndAnswerTokens.from_pretrained("gpt2")


def test_bert_tokenizer(bert_tokenizer_with_context_token):
    assert (
        bert_tokenizer_with_context_token.num_added_special_tokens == 1
    )  # CONTEXT_TOKEN
...
def test_gpt2_tokenizer(gpt2_tokenizer_with_context_and_answer_tokens):
    assert (
        gpt2_tokenizer_with_context_and_answer_tokens.num_added_special_tokens == 4
    )  # PAD_TOKEN, MASK_TOKEN, CONTEXT_TOKEN, ANSWER_TOKEN.
...
def assert_special_tokens_are_preserved(
